...
#pragma warning restore 4014
        ...
        public async Task SubscribeToTickerAsync()
        {
            if (!ActiveSubscriptions.ContainsKey(SubjectNameTicker)) {
                ...
                ActiveSubscriptions.Add(SubjectNameTicker, 
                    WampChannel.RealmProxy.Services.GetSubject(SubjectNameTicker).Subscribe(x => ProcessMessageTicker(x.Arguments)));
            }
        }
        ...
        public async Task SubscribeToTrollboxAsync()
        {
            if (!ActiveSubscriptions.ContainsKey(SubjectNameTrollbox)) {
                ...
                ActiveSubscriptions.Add(SubjectNameTrollbox,
                    WampChannel.RealmProxy.Services.GetSubject(SubjectNameTrollbox).Subscribe(
                        x => ProcessMessageTrollbox(x.Arguments)));
            }
        }
        ...
        private void ProcessMessageTicker(ISerializedValue[] arguments)
        {
            ...
            var marketData = new MarketData {
                PriceLast = priceLast,
                LowestAsk = orderTopSell,
                HighestBid = orderTopBuy,
